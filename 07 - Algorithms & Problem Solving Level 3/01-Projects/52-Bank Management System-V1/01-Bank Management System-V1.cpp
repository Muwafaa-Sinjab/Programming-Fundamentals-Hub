...
// Helper to read a double safely (loops until valid numeric input)
...
// Reads a menu choice with simple validation against non‑numeric input
short getUserChoiceFromMainMenu()
{
    short num = 0;
    while (true)
...
// Splits a string by a delimiter, ignoring empty tokens
vector<string> splitString(string text, string delimiter)
{
...
// Marks a client record for deletion (the file is updated only when saved)
bool markClientForDeleteByAccountNumber(vector<sClientInfo> &vClients, string accountNumber)
{
    for (sClientInfo &c : vClients)
    {
...
// Safely reads a double from cin – loops until valid input
double readDouble(string message)
{
    double value = 0.0;
    while (true)
