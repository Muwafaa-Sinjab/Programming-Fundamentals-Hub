...
int main()
{
    ...
    cout << "Today is: " << dayShortName << ", "
         << date.day << "/" << date.month << "/" << date.year << "\n\n";

    cout << "Is it end week? "
         << (IsEndOfWeek(date) ? "YES" : "NO") << endl;

    cout << "Is it business day? "
         << (IsBusinessDay(date) ? "YES" : "NO") << "\n" << endl;

    cout << "Days until end of week  : "
         << GetDaysUntilEndOfWeek(date) << " Day(s)." << endl;

    cout << "Days until end of month : "
         << GetDaysUntilEndOfMonth(date) << " Day(s)." << endl;

    cout << "Days until end of year  : "
         << GetDaysUntilEndOfYear(date) << " Day(s)." << endl;
...
bool IsEndOfWeek(sDate date)
{
    short dayCode = GetDayOfWeekOrder(date);

    return (dayCode == 5 || dayCode == 6);
}
