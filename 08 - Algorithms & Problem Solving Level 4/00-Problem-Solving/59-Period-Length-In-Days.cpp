...
int main()
{
    ...
    cout << "Period Length (Including End Date) is: "
...
short PeriodLengthInDays(sPeriod period, bool IncludeEndDay = true)
{
    return GetDifferencesInDays(
        period.startDate,
        period.endDate,
        IncludeEndDay
    );
...
bool IsLastDayInMonth(sDate date)
{
    return (date.day == GetNumberOfDaysInMonth(
        date.year,
        date.month
    ));
}
