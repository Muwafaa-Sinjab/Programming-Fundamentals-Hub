...
int main()
{
    ...
    cout << "OverLap Day Count = "
         << GetOverlapDaysCount(p1, p2, true) << endl;
...
int GetOverlapDaysCount(sPeriod p1, sPeriod p2, bool IncludeEndDay)
{
    ...
    sDate overlapStart =
        (CompareDate(p1.startDate, p2.startDate) == After)
        ? p1.startDate
        : p2.startDate;

    // The overlap end is the earlier of the two end dates
    sDate overlapEnd =
        (CompareDate(p1.endDate, p2.endDate) == Before)
        ? p1.endDate
        : p2.endDate;
...
enCompareDateValue CompareDate(sDate d1, sDate d2)
{
    if (IsDateOneBeforeDateTwo(d1, d2))
        return Before;
...
sDate ReadFullDate()
{
    return {ReadYear(), ReadMonth(), ReadDay()};
}